...
for ticks in range(targetTicks):
    ...
    # Pure Lazy. Award a point in a brute forcey way.
    leadDist = 0
    leadDeer = ''
    for name in reindeerlist:
        if reindeerlist[name]['dist'] > leadDist:
            leadDist = reindeerlist[name]['dist']
            leadDeer = name

    reindeerlist[leadDeer]['score'] += 1
...
maxScore = 0
for name in reindeerlist:
    if reindeerlist[name]['score'] > maxScore:
        maxScore = reindeerlist[name]['score']
